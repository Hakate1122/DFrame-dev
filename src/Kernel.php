...
class Kernel
{
    ...
    public function __construct()
    {
        if (!defined('ROOT_DIR') || !defined('INDEX_DIR')) {
            throw new Exception('ROOT_DIR and INDEX_DIR must be defined before initializing framework.');
        }
    }
    ...
    public static function setMaintenanceMode($noEnv = false): void
    {
        ...
        // If $noEnv = true, always enable maintenance
        if ($noEnv === true) {
     ...
     *
     * @param string|null $routeFilePath Optional custom route file path (directory).
     * @return void
     * @throws Exception if the route configuration file is not found or invalid
     */
    private static function initializeRoute($routeFilePath = '')
    {
        // Load route config
        $routeConfig = require(ROOT_DIR . 'config/route.php');
        $routeDir = $routeFilePath ?: ($routeConfig['route_path'] ?? (ROOT_DIR . 'app/Router'));
        $routeFiles = $routeConfig['route_file'] ?? ['web' => 'web.php', 'api' => 'api.php'];

        // Resolve route file paths
        $webRoute = rtrim($routeDir, '/\\') . DIRECTORY_SEPARATOR . ($routeFiles['web'] ?? 'web.php');
        $apiRoute = rtrim($routeDir, '/\\') . DIRECTORY_SEPARATOR . ($routeFiles['api'] ?? 'api.php');

        if (file_exists($webRoute)) {
            require $webRoute;
        } else {
            throw new Exception("Route config not found: " . $webRoute);
        }
    ...
    /**
     * Boots the web application (starts routing).
     *
     * @return void
     */
    ...
    /**
     * Boots the service application (starts service handler).
     *
     * @return void
     */
