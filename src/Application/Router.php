...
class Router
{
     ...
     * @param int|null $httpCode Optional HTTP response code to set when default handler is called.
     * @return static
     */
    public static function default(callable $handler, ?int $httpCode = 404)
    {
        self::$defaultHandler = [
            'handler' => $handler,
            'httpCode' => $httpCode
    ...
    public function runInstance(): void
    {
        ...
        if (self::$defaultHandler) {
            $handlerArr = self::$defaultHandler;
            $handler = is_array($handlerArr) && isset($handlerArr['handler']) ? $handlerArr['handler'] : $handlerArr;
            $httpCode = is_array($handlerArr) && isset($handlerArr['httpCode']) ? $handlerArr['httpCode'] : null;
    ...
    private function callStandardHandler(array $routeData, array $params = []): void
    {
        ...
        foreach ($middleware as $mw) {
            if (is_string($mw)) {
                $result = \Core\Application\Middleware::run($mw, $context);
                if ($result === false) {
                    return;
                }
            } else {
    ...
    private function respondApi(array $routeData, array $params = []): bool
    {
        ...
        http_response_code(404);
        return json_encode(['error' => 'Invalid API route']);
    ...
    /**
     * Normalize handler formats.
     * Supports:
     *  - callable
     *  - [ClassName, 'method']
     *  - 'ClassName@method'  (converted to [ClassName, 'method'])
     *
     * @param mixed $handler
     * @return mixed
     */
